...
def extract_hashrate_series(coin_label: str) -> pd.Series:
    ...
    slug = quote(base)
    url = f"https://bitinfocharts.com/comparison/{slug}-hashrate.html"
    ...
    if not records:
        raise RuntimeError(f"No hashrate records parsed for {coin_label}")
...
def main():
    ...
    # Save CSV
    csv_filename = "hashrates_2015_to_present_bitinfocharts.csv"
    ...
    for coin in COINS.keys():
        ...
        ax.set_title(f"{coin} Hashrate (2015 - Present)")
        ax.set_xlabel("Date")
        ax.set_ylabel("Hashrate (hash/s)")
        fig.autofmt_xdate()
        fig.tight_layout()

        outname = f"{coin.lower().replace(' ', '_').replace('-', '')}_hashrate.png"
    ...
    ax.set_title("Cryptocurrency Hashrates (2015 - Present)")
    ax.set_xlabel("Date")
    ax.set_ylabel("Hashrate (hash/s)")
    ...
    composite_name = "hashrates_composite.png"
