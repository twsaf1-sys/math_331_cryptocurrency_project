...
def extract_hashrate_series(coin_label: str) -> pd.Series:
    ...
    slug = quote(base)
    url = f"https://bitinfocharts.com/comparison/{slug}-price.html"
    ...
    if not records:
        raise RuntimeError(f"No market-rate records parsed for {coin_label}")
...
def main():
    ...
    # Save CSV
    csv_filename = "market_price_2015_to_present_bitinfocharts.csv"
    ...
    for coin in COINS.keys():
        ...
        ax.set_title(f"{coin} Price (2015 - Present)")
        ax.set_xlabel("Date")
        ax.set_ylabel("Market Price (USD)")
        fig.autofmt_xdate()
        fig.tight_layout()

        outname = f"{coin.lower().replace(' ', '_').replace('-', '')}_prices.png"
    ...
    ax.set_title("Cryptocurrency Prices (2015 - Present)")
    ax.set_xlabel("Date")
    ...
    composite_name = "coin_prices_composite.png"
